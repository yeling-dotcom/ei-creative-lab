...
export function MessageInbox({ initialMessages }: { initialMessages: Message[] }) {
  ...
  return <div className="order-list">{messages.map(message => <article className={`order-card message-card ${message.is_read ? "read" : ""}`} key={message.id}><div><div className="order-meta"><span>{new Date(message.created_at).toLocaleString()}</span><span>•</span><span>{message.is_read ? "read" : "new"}</span></div><h3>{message.sender_name}</h3><a href={`mailto:${message.sender_email}`}>{message.sender_email}</a><p>{message.message}</p></div><div className="actions"><a className="button secondary" href={`mailto:${message.sender_email}`}>Reply</a><button onClick={() => mark(message)}>{message.is_read ? "Mark unread" : "Mark read"}</button><button className="danger" onClick={() => remove(message.id)}>Delete</button></div></article>)}</div>;
}
